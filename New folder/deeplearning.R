...
comments$is_neg = ifelse(comments$means_tot < 0, 1, 0)

# comments[,is_neg := ifelse(means_tot < -1, 1, 0)]

df_dl <- comments[,c('comment_date', 'comment_like_count', 'comment_dislike_count', 'comment_description','is_neg')]
...
it_train = itoken(df_dl$comment_description, 
...
# un_comment this for time print(difftime(Sys.time(), t1, units = 'sec'))
...
library(h2o)
h2o.init(nthreads = -1)
model = h2o.deeplearning(y = 'is_neg',
                         training_frame = as.h2o(dtm_train),
...
head(pred[V2 == 1 & is_neg == 0, comment_description],10)
head(pred[V2 == 0 & is_neg == 1,comment_description],10)
head(pred[V2 == 1 & is_neg == 1,comment_description],10)
